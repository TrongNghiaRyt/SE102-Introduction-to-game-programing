...
void CGame::Init(HWND hWnd)
{
	...
	font = MakeFont(L"Press Start", 16);
...
LPD3DXFONT CGame::MakeFont(LPCWSTR name, int size)
{
	...
	//if (d3ddv == NULL)
	//	DebugOut(L"Can't create Font");
	//D3DXCreateFontIndirect(d3ddv, &desc, &f);

	HRESULT h = D3DXCreateFont(d3ddv,     //D3D Device
		size,               //Font height
		0,                //Font width
		...
		&f);         //ppFont
	if (h != S_OK)
		DebugOut(L"Create Font fail");
	return f;
}

void CGame::FontPrint(int x, int y, LPCWSTR text, D3DCOLOR color)
{
...
void CGame::FontPrintInt(int x, int y, int num, D3DCOLOR color)
{
	wchar_t text[_MAX_U64TOSTR_BASE2_COUNT];
	_itow_s(num, text, 10);
