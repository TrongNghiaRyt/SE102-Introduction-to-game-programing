...
void CSprites::AddFromFile(LPDIRECT3DTEXTURE9 tex, LPCWSTR filePath)
{
	...
	DebugOut(L"Add Sprite from file %s\n", filePath);
	file.close();
}
...
void CSprite::Draw(float x, float y, int alpha, bool view)
{
	CGame * game = CGame::GetInstance();
	game->Draw(x, y, texture, left, top, right, bottom, alpha, view);
}
...
void CAnimations::AddFromFile(LPCWSTR filePath)
{
	...
	DebugOut(L"Add animation from file %s\n", filePath);
	file.close();
}
